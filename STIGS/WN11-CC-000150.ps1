<#
.SYNOPSIS
This PowerShell script ensures a password is required on resume from sleep (plugged in) on Windows 11.
...
try {
    ...
    if ($result.$valueName -eq $valueData) {
        Write-Host "SUCCESS: Password is required when the computer wakes (plugged in)." -ForegroundColor Green
    } else {
